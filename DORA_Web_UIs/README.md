...
## Why not RAG — and not a skill either
...
Not a skill either, in the usual sense: a skill puts those same written-down rules back into
a prompt and asks the model to re-derive them every run. Compiling them into code instead buys
byte-identical output across runs, a `rule_id` on every row tracing the verdict to the exact
branch that produced it, and a perturbation suite whose failures are attributable — which is how
three real bugs surfaced. What survives from the skill framing is the shape: one judgement unit
per requirement, a fixed order, verbatim evidence, and an explicit "I don't know" that escalates.
...
crf/                             pipeline package (file-by-file map in ONBOARDING.md)
...
- **[GLOSSARY.md](./GLOSSARY.md)** — every term in the code, the CSVs, and the contract documents, defined in one place.
- **[ONBOARDING.md](./ONBOARDING.md)** — where things live and what to edit when you want to change a behaviour. Start here if you're touching the code.
