...
**And not a skill either** - not in the usual sense. A skill puts the rules in a
prompt and asks the model to follow them each time. But these rules are already
written down: the 105.6 ladder, the metadata gates, the severity table. Executing a
stated rule is free and exact; inferring it is neither. Compiling the rules into code
instead buys three things a prompt can't: byte-identical output across runs, a
`rule_id` on every row tracing the verdict to the exact branch that produced it, and a
perturbation suite whose failures are attributable - which is how three real bugs were
found. What survives from the skill framing is the shape: one judgement unit per
requirement, a fixed order, verbatim evidence, and an explicit "I don't know" that
escalates.
