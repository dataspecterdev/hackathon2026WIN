(() => {
  ...
  function heroNode() {
    ...
    p.textContent =
      "Deterministic checks against the 18-item CC reference checklist — every verdict traceable to the rule that produced it.";
    hero.appendChild(p);
  ...
  const SAMPLE_QUESTIONS = [
    ...
    "Review {pkg} against CC-01, CC-08 and CC-16.",
    "Score {pkg} against the development labels.",
  ];
  ...
  function sampleQuestionsFor(pkg) {
    ...
    if (pkg.startsWith("DEV-")) out.push(`Score ${pkg} against the development labels.`);
    return out;
  }
